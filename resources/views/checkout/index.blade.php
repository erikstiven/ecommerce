<x-app-layout>
    ...
    <div class="-mb-16 text-gray-700" x-data="{ pago: 1, mostrarModal: false }">
        {{-- Formulario de presentación ONLY: evita envío real para no requerir rutas --}}
        <form onsubmit="event.preventDefault()" enctype="multipart/form-data" class="w-full">
                            ...
                            <ul class="divide-y divide-gray-400">
                                <li>
                                    ...
                                    <div class="p-4 bg-gray-100 text-center border-t border-gray-400"
                                        ...
                                        <p class="mt-2">Luego de dar click en "Pagar" serás redirigido a la pasarela
                                            de pago</p>
                                    </div>
                                </li>
                                ...
                                <li>
                                    ...
                                    <div x-show="pago == 2" x-cloak
                                        ...
                                        {{-- INPUT para subir comprobante (solo UI) --}}
                                        <div class="max-w-md mx-auto space-y-1">
                                            <label class="block text-sm font-medium text-gray-700 mb-1">Adjuntar
                                                comprobante (opcional)</label>
                                            <input type="file" id="deposit_proof" accept="image/*,.pdf"
                                                capture="environment" class="block w-full text-sm text-gray-700">
                                            <div id="deposit-preview" class="mt-3"></div>
                                            <p class="text-xs text-gray-500 mt-1">Formato JPG/PNG o PDF. Máx. 5MB. (Demo
                                                UI, no se sube)</p>
                                        </div>
                                    </div>
                                </li>
                ...
                <div class="cols-span-1">
                    <div class="lg:max-w-[40rem] py-12 px-4 lg:pl-8 sm:pr-6 lg:pr-8 mr-auto">
                        ...
                        <div class="flex justify-between mb-2">
                            ...
                            <p>$./ {{ number_format(Cart::instance('shopping')->subtotal() + 5, 2) }}</p>
                        </div>

                        <div class="flex justify-between">
                            <p>Subtotal</p>
                            <p>$./ {{ number_format(Cart::instance('shopping')->subtotal(), 2) }}</p>
                        </div>
                        ...
                        {{-- Botón para PayPhone (muestra modal) --}}
                        <div class="mt-4" x-show="pago == 1">
                            <button type="button" @click="mostrarModal = true"
                                class="btn btn-gradient-purple text-white rounded w-full">
                                Confirmar y Pagar
                            </button>
                        </div>

                        {{-- Botón para depósito (UI only: no envía) --}}
                        <div class="mt-4" x-show="pago == 2">
                            <button type="button"
                                @click.prevent="alert('Demo: la orden se generaría aquí. Conecta la lógica cuando estés listo.')"
                                class="btn btn-gradient-purple text-white rounded w-full">
        ...
        <div x-show="mostrarModal" x-cloak @click.self="mostrarModal = false"
            ...
            <div class="bg-white p-6 rounded shadow-lg w-full max-w-md text-center">
                ...
                <button @click="mostrarModal = false"
                    class="mt-4 text-sm text-gray-600 hover:text-red-500">Cancelar</button>
            </div>
        </div>
    </div>

    @push('js')
        {{-- Payphone (solo UI render) --}}
        ...
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                // Render UI button (no inicia pago real porque es demo)
                ...
                if (buttonContainer) {
                    try {
                        ...
                        // si no quieres que falle la vista en demo, solo logueamos
                        console.warn('PayPhone UI (demo) no disponible', err);
                    }
                }

                // Preview de comprobante (imagen o pdf) - UI only
                const fileInput = document.getElementById('deposit_proof');
